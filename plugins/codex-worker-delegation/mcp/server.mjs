...
async function handle(msg) {
  if (msg.method === 'initialize') return reply(msg.id, { protocolVersion: msg.params?.protocolVersion || '2025-06-18', capabilities: { tools: {} }, serverInfo: { name: 'codex-worker-delegation', version: '3.1.1' } });
  ...
  if (msg.method === 'tools/list') return reply(msg.id, { tools: [
    ...
    { name: 'delegate_worker', description: 'Start the Web-selected Worker or Verifier and return its task ID, live state, heartbeat, progress events, and result. Use profile=quick for short probes and profile=standard for implementation work. The control plane automatically reviews each lease checkpoint: concrete recent progress renews within the hard cap; real execution with a temporary heartbeat-only interval receives one bounded grace; repeated heartbeat-only or stalled work is cancelled safely. Under MAIN the server rejects delegation. If the returned task is still running after the wait window, continue with worker_status; manual extend/cancel remains root-control-only.', inputSchema: { type: 'object', properties: { task: { type: 'string', minLength: 1, maxLength: MAX_TASK_LENGTH }, role: { type: 'string', enum: ['worker', 'verifier'] }, cwd: { type: 'string', maxLength: 4096 }, mode: { type: 'string', enum: ['AUTO', 'DELEGATE', 'MAIN'] }, profile: { type: 'string', enum: ['quick', 'standard'] }, timeoutMs: { type: 'number', minimum: 1000, maximum: 900000 }, maxTotalTimeoutMs: { type: 'number', minimum: 1000, maximum: 3600000 }, waitMs: { type: 'number', minimum: 0, maximum: 170000 } }, required: ['task'], additionalProperties: false } },
    ...
    { name: 'worker_extend', description: 'Root-control fallback: after worker_status reports reviewDue=true and the work is demonstrably on scope and safe, extend that one Worker or Verifier lease. Automatic supervision normally handles this checkpoint. MAIN mode rejects lease extension.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', minLength: 8, maxLength: 128 }, extraMs: { type: 'number', minimum: 1000, maximum: 900000 }, reason: { type: 'string', maxLength: 500 } }, required: ['taskId'], additionalProperties: false } },
    { name: 'worker_cancel', description: 'Root-control fallback: cancel one running Worker or Verifier task by task ID after inspecting worker_status. Automatic supervision cancels stalled or missing-evidence paths; this manual action terminates only that task App Server and is idempotent for terminal tasks.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', minLength: 8, maxLength: 128 }, reason: { type: 'string', maxLength: 500 } }, required: ['taskId'], additionalProperties: false } }
  ] });
...
async function state() { const file = path.join(baseDir(), 'state.json'); try { const parsed = JSON.parse(await fs.readFile(file, 'utf8')); if (!['AUTO', 'DELEGATE', 'MAIN'].includes(parsed?.mode)) throw new Error(`invalid mode ${String(parsed?.mode)}`); return parsed; } catch (error) { const wrapped = new Error(`delegation control state is unavailable: ${error.message}`); wrapped.code = 'DELEGATION_STATE_UNAVAILABLE'; throw wrapped; } }
...
async function delegateWorker(args) {
  ...
  // Explicitly whitelist the payload. MCP schema validation is advisory at the
  // protocol boundary and must not become a way to smuggle sandbox or unknown
  // control fields into the privileged local Worker endpoint.
