...
async function handle(msg){
  ...
  if(msg.method==='tools/list')return reply(msg.id,{tools:[
    ...
    {name:'delegate_worker',description:'Run the Web-selected worker or verifier. Any route involving a third-party provider uses an explicit Codex App Server thread; built-in OpenAI-to-OpenAI returns a native-subagent instruction.',inputSchema:{type:'object',properties:{task:{type:'string'},role:{type:'string',enum:['worker','verifier']},cwd:{type:'string'},mode:{type:'string',enum:['AUTO','DELEGATE','MAIN']}},required:['task'],additionalProperties:false}}
  ]});
  if(msg.method==='tools/call'&&msg.params?.name==='delegation_status'){const s=await state();return toolReply(msg.id,{mode:s.mode,routing:s.routing||null,provider:s.provider&&{name:s.provider.name,baseUrl:s.provider.baseUrl,protocol:s.provider.protocol},protocolCache:s.protocolCache,installed:Boolean(s.installed)})}
...
async function delegateWorker(args){if(!args.task?.trim())throw new Error('task is required');const token=(await fs.readFile(path.join(baseDir(),'gateway.token'),'utf8')).trim();const port=Number(process.env.CWD_PORT||8788);const response=await fetch(`http://127.0.0.1:${port}/internal/worker/run`,{method:'POST',headers:{authorization:`Bearer ${token}`,'content-type':'application/json'},body:JSON.stringify(args),signal:AbortSignal.timeout(190000)});const text=await response.text();let body;try{body=JSON.parse(text)}catch{body={error:text}}if(!response.ok)throw new Error(body?.error?.message||body?.error||`worker request failed (${response.status})`);return body}
