...
function isDelegationMcp(tool=''){const name=String(tool);return name.includes('codex-worker-delegation')||name.endsWith('delegate_worker')||name.endsWith('delegation_status')||name.endsWith('worker_status')||name.endsWith('worker_extend')||name.endsWith('worker_cancel')}
...
async function readState(){try{return JSON.parse(await fs.readFile(stateFile(),'utf8'))}catch(e){if(e.code==='ENOENT')return null;throw e}}
