...
The standard lease defaults to 15 minutes; `quick` starts at 2 minutes and is hard-capped at 10 minutes. No later than 90 seconds before the current deadline, the task emits `worker.review_due` and the control plane performs an automatic evidence review. Recent meaningful lifecycle progress with a healthy heartbeat renews the same App Server notification deadline, by at most 15 minutes for standard work or 2 minutes for quick work, and never beyond the hard total cap (60 minutes standard, 10 minutes quick). A task that has entered real execution but temporarily emits only heartbeats receives one bounded grace review; repeated heartbeat-only, stalled, unavailable, or exhausted work is cancelled at the task boundary. Every automatic decision records `decision`, `evidence`, ages, reason, and counts in the task event stream and redacted audit log. The Web UI displays the lease, evidence, and decision but has no cancel or extend action. Root-control `worker_extend` / `worker_cancel` remain available as a fallback when automatic review is disabled or when the root explicitly takes over.
