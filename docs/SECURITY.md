...
- Worker lease renewal is also root-control-only: `worker_extend` is accepted only when the task reports its scheduled `reviewDue` checkpoint, updates the matching App Server turn deadline, and cannot exceed the per-renewal or hard total runtime cap. A heartbeat is only liveness evidence; the root must inspect progress and scope before renewing.
