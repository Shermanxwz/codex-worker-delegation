...
test('catalog-wide failures are advisory when every required core check passed',()=>{const r=classifyProductionSeal({checks:[...core(),bad(catalog),bad(desktopVisibility),bad(desktopBinding)]});assert.equal(r.coreStatus,'CORE_SEALED');assert.equal(r.catalogStatus,'CATALOG_ADVISORY');assert.equal(r.catalogAdvisories.length,1)});
