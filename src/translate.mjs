...
function chatContent(content) {
  if (typeof content === 'string') return content;if (!Array.isArray(content)) return '';
  const parts = [];for (const part of content) {if (['input_text', 'output_text', 'text'].includes(part?.type)) parts.push({ type: 'text', text: part.text || '' });else if (part?.type === 'input_image' && part.image_url) parts.push({ type: 'image_url', image_url: { url: part.image_url } });}
  if (parts.length === 1 && parts[0].type === 'text') return parts[0].text;return parts.length ? parts : textFromContent(content);
}

export function responsesToChat(body, { forwardReasoningEffort = false } = {}) {
  const messages = [];let pendingAssistant = null;
  const flushAssistant = () => {if (!pendingAssistant) return;if (!pendingAssistant.tool_calls?.length) delete pendingAssistant.tool_calls;messages.push(pendingAssistant);pendingAssistant = null;};
  ...
  for (const item of input) {
    if (item?.type === 'message' || (!item?.type && item?.role)) {const role = item.role === 'developer' ? 'system' : item.role;if (role === 'assistant') {flushAssistant();pendingAssistant = { role, content: chatContent(item.content) };} else {flushAssistant();messages.push({ role: role || 'user', content: chatContent(item.content) });}}
    else if (item?.type === 'function_call') {pendingAssistant ||= { role: 'assistant', content: null, tool_calls: [] };pendingAssistant.tool_calls ||= [];pendingAssistant.tool_calls.push({ id: item.call_id || item.id, type: 'function', function: { name: item.name, arguments: typeof item.arguments === 'string' ? item.arguments : JSON.stringify(item.arguments || {}) } });}
    else if (item?.type === 'function_call_output') {flushAssistant();messages.push({ role: 'tool', tool_call_id: item.call_id, content: typeof item.output === 'string' ? item.output : JSON.stringify(item.output ?? '') });}
  }
  flushAssistant();
  ...
  const result = { model: body.model, messages, stream: body.stream !== false };if (tools.length) result.tools = tools;if (body.tool_choice !== undefined) result.tool_choice = mapToolChoice(body.tool_choice);if (body.max_output_tokens) result.max_tokens = body.max_output_tokens;if (result.stream) result.stream_options = { include_usage: true };if (forwardReasoningEffort && body.reasoning?.effort) result.reasoning_effort = body.reasoning.effort;return result;
}
function mapToolChoice(choice) {if (typeof choice === 'string') return choice;if (choice?.type === 'function' && choice.name) return { type: 'function', function: { name: choice.name } };return choice;}
